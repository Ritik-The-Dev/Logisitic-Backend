// middlewares/adminAuthenticated.ts
...
export const adminAuthenticated = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
        ...
        if(user.type != "admin"){
            res.status(401).json(errorHandler('User type invalid'));
            return;
        }
        req.user = user;
        next();
        ...
        res.status(500).json(errorHandler('Unexpected error', error.message));
    }
};
